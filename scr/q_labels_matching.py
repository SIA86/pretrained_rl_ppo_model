...
""" 
1. Исполнение по цене открытия следующего бара: решения на баре *t* исполняются на ``Open[t+1]`` — обеспечивает корректную причинность.
2. Односторонний режим и действия: отдельно работаем «только long» или «только short»; действия — ``Open``, ``Close``, ``Hold``, ``Wait``.
   Реверсы запрещены (Close сразу за которым следует Open).
3. Абсолютные Q‑метки: важен сохранённый масштаб выигрыша, поэтому не делим на σ, √Δt и не применяем построчные нормировки.
4. Единая временная база: внутри позиции сравнение ведём с ``t+1`` —
   ``Close = 0`` (после выхода прирост нулевой), ``Hold = Ret(t+1 → TGT)``.
   Это избегает преждевременных выходов из-за разного горизонта.
5. Разметка TD‑λ: смесь n‑step горизонтов (TF‑style). Снижает дисперсию меток и согласуется с PPO/A2C.
6. Нормализация/клип: допускается только глобальный winsorize/клип/робаст‑скейл по train (единый для всех действий),
   чтобы стабилизировать обучение и подобрать температуру τ для softmax. Построчные нормировки не применять.
7. Таргеты для SL → PPO: строим ``Y = softmax(Q/τ)`` с масками валидности (advantage‑нормировка нежелательна, т.к. теряет глобальный контекст,
   важный для калибровки PPO). Value‑head можно инициализировать как ``Vτ(s) = τ * log Σ_a exp(Q_a/τ)`` или ``max_a Q_a``.
"""
